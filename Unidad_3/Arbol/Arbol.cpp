...
void busquedaBinaria(NodoArbol* raiz, int idBuscado) {
    ...
    if (raiz->dato.id == idBuscado) {
        ...
        cout << "ID: " << raiz->dato.id
             << " Etiqueta: " << raiz->dato.etiqueta
             << endl;
        return;
    }

    if (idBuscado < raiz->dato.id) {
        cout << "-> Voy a la izquierda" << endl;
        busquedaBinaria(raiz->izq, idBuscado);
    }
    else {
        cout << "-> Voy a la derecha" << endl;
        busquedaBinaria(raiz->der, idBuscado);
    }
}
